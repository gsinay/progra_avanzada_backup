        ...
        for row in range(self.rows):
            for column in range(self.columns):
                if row == 0 or row == LARGO_GRILLA - 1 or column == 0 or column == ANCHO_GRILLA - 1:
                    elemento = QLabel()
                    elemento.setPixmap(QPixmap(os.path.join('sprites', 'Elementos', 
                                                            'bordermap.png')).scaled(50,50))
                else:
                    elemento = QLabel()
                ...
                if len(grilla[fila][columna]) == 1:
                    if grilla[fila][columna][0] == "luigi":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Personajes', 'luigi_front.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "fantasma_vertical":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Personajes', 'red_ghost_vertical_1.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "fantasma_horizontal":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Personajes', 'white_ghost_rigth_1.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "fuego":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Elementos', 'fire.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "roca":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Elementos', 'rock.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "pared":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Elementos', 'wall.png')).scaled(50,50)
                    elif grilla[fila][columna][0] == "estrella":
                        pixmap = QPixmap(os.path.join(
                            'sprites', 'Elementos', 'osstar.png')).scaled(50,50)
